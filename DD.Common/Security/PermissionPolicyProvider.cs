...
namespace DD.Common.Security
{
    public class PermissionPolicyProvider : IAuthorizationPolicyProvider
    {
        ...
        // (like [PermissionAuthorize] in this sample)
        public Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            if (policyName.StartsWith(POLICY_PREFIX, StringComparison.OrdinalIgnoreCase))
            {
                IEnumerable<Permissions> permissions = policyName.Substring(POLICY_PREFIX.Length)
                    .Split(',').Select(p => (Permissions) Enum.Parse(typeof(Permissions), p));
